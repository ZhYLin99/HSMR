...
from zjrr import ZJRR
from modeling.embedding_mask_head import EmbeddingMaskHead
from modeling.mask_guided_baseline import MaskGuidedBaseline
...
if __name__ == '__main__':
    ...
    if os.path.exists(output):
        ...
        sys.exit(0)
    out_put_father_folder = output.strip("embedding_wb.pkl")
    if not os.path.exists(out_put_father_folder):
        os.mkdir(out_put_father_folder)
    cfg = setup_cfg(args)

    cfg.defrost()
    cfg.MODEL.BACKBONE.PRETRAIN = False
    model = build_model(cfg)
    # KeyError: "No object named 'MaskGuidedBaseline' found in 'META_ARCH' registry!"
    # Solve:
    # from zjrr import ZJRR
    # from modeling.embedding_mask_head import EmbeddingMaskHead
    # from modeling.mask_guided_baseline import MaskGuidedBaseline
