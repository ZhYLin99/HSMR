...
class ZJRR(ImageDataset):
    ...
    def __init__(self, root='', **kwargs):
        ...
        self.mask_root = os.path.abspath(os.path.join(self.image_root, os.path.pardir, "hs_bbxes"))
    ...
    def process_train(self, txt_path):
        ...
        for txt in txt_path:
            ...
            for line in open(txt).readlines():
                ...
                mask_path = os.path.join(self.mask_root, line+'.txt')
                ...
                d.append((img_path, pid, camid, mask_path))
            data.append(d)
        return data
    ...
    def map_pid_camid(self, query, gallery):
        ...
        query = [(x[0], pid_dict[x[1]], 0, x[3]) for i,x in enumerate(query)]
        gallery = [(x[0], pid_dict[x[1]], 1, x[3]) for i,x in enumerate(gallery)]
