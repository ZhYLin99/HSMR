...
benchmarks = ['iros2022-fisheye-similar-test', 'iros2022-fisheye-original-test']
...
if __name__ == '__main__':
    for benchmark in benchmarks:
        # summary whole body
        # trackers = glob.glob(os.path.join(trackers_folder, benchmark, "origin_deep_sort_wb_*"))
        trackers = glob.glob(os.path.join(trackers_folder, benchmark, "deep_sort_wb_*"))
        res = summaries(trackers)
        for metric in res:
            print("Whole body", benchmark, metric)
            print(res[metric])

        # summary header shoulder
        # trackers = glob.glob(os.path.join(trackers_folder, benchmark, "origin_deep_sort_hs_*"))
        # res = summaries(trackers)
        # for metric in res:
        #     print("Header shoulder", benchmark, metric)
        #     print(res[metric])
